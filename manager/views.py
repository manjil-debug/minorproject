    ...
    if request.session.has_key('u_name'):
        if request.method == "POST":
            ...
            if num >= 10:
                ...
                ph_validate = 0
                messages.error(request,'There must be 10 digits in your number')

            if customer_requests.objects.filter(date=date, venue=venue).exists():
                date_validate = 0
                messages.error(request, 'The Date is already booked for the venue please select a different date '
                                        'or venue')
            else:
                date_validate = 1
            ...
            if ph_validate == 1 and date_validate == 1:
                ...
                if obj:
                    messages.success(request, 'Your Booking request has been successfully sent please wait for us '
                                              'to contact you')
                else:
                    messages.error(request, 'Your Booking request was not-Successful please try again later')
